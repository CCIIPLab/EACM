...
class MEMGRUCell(RNNCell):
    ...
    def __call__(self, inputs, state, emotion, imemory, scope=None):
        """Gated recurrent unit (GRU) with nunits cells and internal memory."""
        params = []
        ...
        with vs.variable_scope(scope or 'GRUCell'):    # "GRUCell"
            with vs.variable_scope("Emotion_Imemory"):  # "Internal memory"
                # We start our internal memory without a bias term.
                _r, _u, _c = array_ops.split(1, 3, _linear(params, 3 * self._num_units, False))
